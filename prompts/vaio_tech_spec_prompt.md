...
非エンジニアやビジネスユーザーが **Gemini Web UI (gemini.google.com)** や **Google AI Studio** のチャット画面に VAIO ノートパソコンの仕様一覧表画像（仕様一覧① P.25-26、仕様一覧③ P.29-30など）をアップロードし、以下のプロンプトをコピペして実行することで、**CPU・メモリ・ストレージ・駆動時間・寸法質量付きの標準仕様JSON (technical_specifications_vaio.json)** を出力できます。
...
4. **JSONデータをコピーして保存する**
   - 出力された JSON コードブロックの右上にある **「コピー」ボタン** をクリックし、`technical_specifications_vaio.json` として保存します。
